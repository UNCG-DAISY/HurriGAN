...
Adversarial Networks conditioned by city
...
Options:
--ROOT_FOLDER=<ROOT_FOLDER>   Name of the folder in the AWS S3 bucket in which the images are stored [default: data/exterior.zip]
--METHOD=<METHOD>   Loading data method (ram or data_generator) [default: ram]
[--IMG_SIZE=<IMG_SIZE>]    Image size [default: 140]
...
def main(root_folder, method, img_size, batch_size, epochs,checkpoint):
    ...
    # Splitting the cities as a list
    cities_str = "Boston,Charleston,Chicago,Denver,Houston,LosAngeles,Miami,Morgantown,Portland,Seattle"
    ...
    image_in_batches, steps_per_epoch = load_data(root_folder, cities, batch_size, 
                                                 img_width, img_height, method)
    
    print("Step 1 - DONE!", end='\n')

    print("Step 2 - Building the GAN", end='\n')
    make_gan(image_in_batches, steps_per_epoch, batch_size, epochs, img_width, img_height, cities,checkpoint)
    #return image_in_batches,steps_per_epoch
    print("Step 2 - DONE!", end='\n')

def make_gan(image_in_batches, steps_per_epoch, batch_size, epochs, img_width, img_height, city_list,checkpoint):
    """
    ...
    gans = cdcgans(img_size = img_width, noise_dim = 100, n_classes = len(city_list),
               lr_g = 0.0002, lr_d = 0.0002, beta1 = 0.5,
               num_examples_to_generate = len(city_list),
    ...
    print("Step 2.2 - Creating the example array")
    
    print("Step 2.3 - Training the model")
    gans.train(dataset = image_in_batches, steps_per_epoch = steps_per_epoch, labels = city_list, 
            saving_gap = 50,
            save_img = True, img_path = 'results/gan/examples', 
            save_loss_batch = True, save_loss_epoch = True, loss_path = 'results/gan/loss' )
    
    print("Step 2.4 - Saving weights")
    gans.generator.save("results/gan/weights/generator_ext.h5")

    
#### This portion is for loading the data ####

def load_data(root_folder, cities, batch_size, img_width, img_height, method):
    '''
    ...
    if str.lower(method) == 'ram':
      X,y = get_data(root_folder, cities, 
    ...
    # load data as image generator
      ...
      image_generator = image_data_generator(root_folder, cities, 
                                           batch_size, img_width, img_height)
...
def get_data(root_folder, cities, batch_size,
                        img_width, img_height):
    '''
    store RGB images and city labels in numpy array
    Parameters
    -------------------
    root_folder (str):
        Location of the input folder in the s3 bucket
    ...
    archive = ZipFile(root_folder)
    n = 0
    for entry in archive.infolist():
        with archive.open(entry) as file:

            filename = entry.filename
            slash_position = filename.find("/")
            city = filename[:slash_position]
            ...
            if img.mode == 'RGB':
                ...
                city_list.append(le.transform([city]))
            n+=1

            if n % 10000 == 0:
              print(f"loading image {n} out of {len(archive.infolist())}")
              #break #sample data
                
    return (np.array(img_list), np.array(city_list).reshape((-1, 1)))
...
## load data as image generator ##
def image_data_generator(root_folder, cities, batch_size,
                        img_width, img_height):
    '''
    ...
    while True:    
      for entry in archive.infolist():
          with archive.open(entry) as file:
              ...
              if img.mode == 'RGB':
                  img_list.append(np.asarray(img))
                  city_list.append(city_encoded)
                  n+=1

              if n >= batch_size:
                yield (np.array(img_list)/127.5 - 1, np.array(city_list))
                img_list = []
                city_list = []
                n = 0
...
def get_steps(root_folder, batch_size):
    ...
    generator_len = len(archive.infolist()) 
